...
public class Walker {
    ...
    public Tile getNextTile(Tile[] t) {
        ...
        for (int i = t.length - 1; i >= 0; i--) {
            if (t[i].equals(nextTile)) {                                    //This is the index at which the suggested next tile resides
                ...
                System.out.println("Breaking due to suspected broken path");
            	...
            	index = i+1;//Ignoring above and assuming path is correct. 
            	if (t[i].distanceTo(p)<2) {
            		index = i;
            	}
            	break;
            }
    ...
    public boolean handleObstacle(Tile[] t) {
        ...
        if (nextTile.matrix(ctx).reachable()) {
        	System.out.println("Next tile not reachable.");
            return false;
        }
    ...
    public boolean walkPath(Tile[] t) {
    	if (t[t.length-1].distanceTo(ctx.players.local()) < 5) {
    		return true;
    	}
